...
RED.deploy = (function() {
    ...
    var deploymentTypes = {
        ...
        "rwire":{img:"red/images/deploy-flows-o.svg"}
    }
    ...
    async function  sendFlow(data){
        console.log(JSON.stringify({flows:data}))
            try {
                const makeProjectResponse = await fetch(`https://ceres.epi.it.matsue-ct.ac.jp/rwire/project`, {
                    method: "POST",
                    headers: {
                ...
                if (!makeProjectResponse.ok) {
                    throw new Error(`HTTP error! status: ${makeProjectResponse.status}`);
                }

                const projectRes = await makeProjectResponse.json();
                console.log("サーバーからの応答:", projectRes);
                console.log(projectRes.data.projectId);
                const projectId = projectRes.data.projectId;
                const covertResponse = await fetch(`https://ceres.epi.it.matsue-ct.ac.jp/rwire/project/${projectId}/convert`, {
                    method: "POST",
                });
                const convertResult = await covertResponse.json();
                console.log(convertResult);
                console.log(convertResult.data.mainCode);
                
                //window.open(`http://localhost:5173/?id=${compileRes.data.projectId}`, '_blank');
                const res = await fetch(
                    ...
                    body: JSON.stringify({
                        code: convertResult.data.mainCode,
                    }),
                    },
                );
                if (!res.ok) {
    ...
    function init(options) {
        ...
        if (type == "default") {
            ...
            const mainMenuItems = [
                    ...
                    {id:"deploymenu-item-rwire",toggle:"deploy-type",icon:"red/images/deploy-nodes.svg",label:RED._("deploy.rwire"),sublabel:RED._("deploy.rwireDesc"),onselect:function(s) { if(s){changeDeploymentType("rwire")}}},
                    null
            ]
    ...
    function save(skipValidation, force) {
        ...
        deployInflight = true;
        shadeShow();
        if(deploymentType === "rwire")
        {   
            console.log("rwire");
            const activeWorkspace = RED.workspaces.active();
            console.log(activeWorkspace)
            const flow = nns.filter(function(node){
                if(node.type === "tab" || node.type === "subflow")
                {
                    return node.id === activeWorkspace;
                }
                return node.z === activeWorkspace;
            }
            )
            console.log(flow);
            sendFlow(flow);
        }else if(deploymentType === "flows")
        {
            console.log("flows")
            console.log(data);
        }
