...
var RED = (function() {
    ...
    function loadEditor() {
        ...
        $('<li><a id="red-ui-header-button-custom" class="button" href="#" title="Kani-Board Build"><i class="fa fa-flag" style="color: #2ecc71;"></i></a></li>').prependTo(".red-ui-header-toolbar");

        $("#red-ui-header-button-custom").on("click", async function(e) {
            e.preventDefault();
            ...
            try {
                ...
                nodes = nodes.concat(
                    RED.nodes.junctions(activeWorkspace)
                );
                ...
                RED.notify("プロジェクトを送信中...", { type: "info", timeout: 2000 });
                const response = await fetch(
                    "https://ceres.epi.it.matsue-ct.ac.jp/rwire/project",
                    {
                        method: "POST",
                        headers: {
                ...
                const convertResponse = await fetch(
                    `https://ceres.epi.it.matsue-ct.ac.jp/rwire/project/${projectId}/convert`,
                    {
                        method: "POST"
                    }
                );
                ...
                if (!res.ok) {
                    RED.notify("アップロードに失敗しました。", { type: "error" });
                    return;
                }
                ...
                console.log(json);
                RED.notify("コンパイルが成功しました！", { type: "success", timeout: 3000 });
                window.open(
                ...
                RED.notify("送信に失敗しました。", { type: "error" });
